...
export class RecurrenceEngineRRULE {
  ...
  getNextOccurrence(task: Task, from: Date): Date | null {
    ...
    try {
      ...
      return this.applyFixedTime(next, task.recurrence);
    } catch (error) {
  ...
  getOccurrencesBetween(task: Task, from: Date, to: Date): Date[] {
    ...
    try {
      ...
      // Apply fixed time if specified
      return occurrences.map(date => this.applyFixedTime(date, task.recurrence!));
    } catch (error) {
  ...
  private getRRule(task: Task): RRule {
    const cacheKey = `${task.id}:${task.recurrence!.rrule!}`;
    ...
    const rruleString = task.recurrence!.rrule!;
    const normalized = rruleString.startsWith('RRULE:') ? rruleString : `RRULE:${rruleString}`;
    ...
    // Set dtstart from recurrence if available, otherwise use task's dueAt or createdAt
    if (task.recurrence?.referenceDate) {
      options.dtstart = new Date(task.recurrence.referenceDate);
    } else if (task.dueAt) {
      options.dtstart = new Date(task.dueAt);
    } else if (task.createdAt) {
      options.dtstart = new Date(task.createdAt);
    }
  ...
  private getBaseDate(task: Task, from: Date): Date {
    ...
    if (baseOnToday) {
      // For baseOnToday, calculate from the reference date (completion date)
      return from;
    }
  ...
  /**
   * Apply fixed time to a date if specified in recurrence
   */
  private applyFixedTime(date: Date, recurrence: Recurrence): Date {
    if (!recurrence?.time) {
      return date;
    }

    try {
      const parts = recurrence.time.split(':').map(Number);
      const hours = parts[0];
  ...
  private legacyFrequencyToRRule(frequency: Frequency, dtstart: Date): string {
    ...
    // @ts-expect-error - Legacy Frequency type may have optional weekdays property
    if (frequency.type === 'weekly' && frequency.weekdays && frequency.weekdays.length > 0) {
      ...
      const byDay = frequency.weekdays.map((d: number) => dayMap[d]).join(',');
    ...
    if (frequency.type === 'yearly') {
      // @ts-expect-error - Legacy Frequency type may have optional month property
  ...
  private createLegacyTask(frequency: Frequency, dueAt: Date, options?: { whenDone?: boolean; timezone?: string }): Task {
    return {
      ...
      enabled: true,
      // @ts-expect-error - Legacy properties not in current Frequency type
  ...
  calculateNext(
    ...
    // @ts-expect-error - Legacy Frequency may have rruleString property
    if (!freq.rruleString) {
      // @ts-expect-error - Adding rruleString to legacy Frequency
      freq.rruleString = this.legacyFrequencyToRRule(freq, currentDue);
      // @ts-expect-error - Adding dtstart to legacy Frequency
