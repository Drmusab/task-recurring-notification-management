...
export class RecurrenceExplainer {
  ...
  explain(
    ...
    const ruleString = task.recurrence?.rrule || '';
    ...
    // Step 5: Frequency
    const freqName = options.freq !== undefined ? this.getFrequencyName(options.freq) : 'Unknown';
    steps.push({
      step: stepNum++,
    ...
    // Step 8: Timezone
    const timezone = options.tzid || task.recurrence?.timezone || task.timezone || getUserTimezone();
    steps.push({
      step: stepNum++,
    ...
    if (task.recurrence?.time && resultDate) {
      steps.push({
        step: stepNum++,
        description: 'Apply fixed time',
        value: `Time set to ${task.recurrence.time}`
      });
    }
  ...
  summarize(rruleString: string, dtstart?: Date): string {
    try {
      ...
      if (parsed instanceof RRule) {
        ...
        // Must be RRuleSet
        const rrules = (parsed as RRuleSet).rrules();
  ...
  private getRecurrenceMode(task: Task): RecurrenceMode {
    ...
    return baseOnToday ? 'whenDone' : 'fixed';
  }
