...
export interface RecurrenceSettings {
  ...
  // ===== Phase 2: RRule-First Mode =====
  
  /** Use RRule by default for new tasks (Phase 2) */
  useRRuleByDefault: boolean;
  
  /** Auto-migrate legacy tasks to RRule on edit (Phase 2) */
  autoMigrateOnEdit: boolean;
  
  /** Show upgrade button for legacy tasks (Phase 2) */
  showUpgradeButton: boolean;
  
  /** Automatically run migration on plugin load (opt-in) */
...
export const DEFAULT_SETTINGS: PluginSettings = {
  ...
  recurrence: {
    ...
    // Phase 2: RRule-First Mode
    useRRuleByDefault: true,       // NEW: Default to RRule for new tasks
    autoMigrateOnEdit: true,       // NEW: Auto-migrate on edit
    showUpgradeButton: true,       // NEW: Show upgrade button in UI
    autoMigrateOnLoad: false,      // NEW: Don't auto-migrate on load (user chooses)
  },
  dependencies: {
    warnOnCycles: true,
    autoValidate: true,
