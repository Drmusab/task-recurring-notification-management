...
export interface QueryAST {
  filters: FilterNode[];
  sort?: SortNode;
  sortFields?: SortField[]; // NEW: Multi-field sorting
  group?: GroupNode;
  limit?: number;
  explain?: boolean;
...
export class QueryParser {
  ...
  private parseSortInstruction(line: string): SortNode {
    // Support multi-field sorting: "sort by urgency, priority reverse, due"
    const sortPrefix = 'sort by ';
    if (!line.toLowerCase().startsWith(sortPrefix)) {
      throw new QuerySyntaxError(
        `Invalid sort instruction: "${line}"`,
        this.line,
        this.column,
        'Use format: "sort by FIELD" or "sort by FIELD1, FIELD2, ..."'
      );
    }
    
    const fieldsStr = line.substring(sortPrefix.length).trim();
    ...
    for (const fieldPart of fieldParts) {
      ...
      if (!match) {
        throw new QuerySyntaxError(
          ...
          this.line,
          this.column,
          'Use format: "FIELD" or "FIELD reverse"'
        );
      }
    ...
    // For backward compatibility, if only one field, return it as legacy SortNode
