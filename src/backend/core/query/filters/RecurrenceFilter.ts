// @ts-nocheck
import { Filter } from "@backend/core/query/filters/FilterBase";
import type { Task } from '@backend/core/models/Task';

export class RecurrenceFilter extends Filter {
  constructor(private negate = false) {
    super();
  }

  matches(task: Task): boolean {
    // A task is recurring if it has a frequency defined
    const isRecurring = task.frequency && task.frequency.type !== 'once';
    return this.negate ? !isRecurring : !!isRecurring;
  }

  // Phase 1: Query Enhancement - Explanation Support
  explain(): string {
    return this.negate ? 'is NOT recurring' : 'is recurring';
  }

  explainMatch(task: Task): string {
    if (this.negate) {
      return `Task "${task.name}" is not recurring (frequency: ${task.frequency?.type || 'none'})`;
    }
    return `Task "${task.name}" is recurring (frequency: ${task.frequency?.type || 'once'})`;
  }

  explainMismatch(task: Task): string {
    if (this.negate) {
      return `Task "${task.name}" is recurring (frequency: ${task.frequency?.type || 'once'}) - expected non-recurring`;
    }
    return `Task "${task.name}" is not recurring (frequency: ${task.frequency?.type || 'none'}) - expected recurring`;
  }
}
