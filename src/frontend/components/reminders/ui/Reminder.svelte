...
<main>
  ...
  <div class="reminder-actions">
    <button class="mod-cta" on:click={onDone} bind:this={doneButton}>
      <IconText icon="check-small" /><span>{$t('reminders.done')}</span>
    </button>
    <button on:click={onMute}>
      <IconText icon="minus-with-circle" text={$t('reminders.mute')} />
    </button>
    <select
    ...
    >
      <!-- placeholder -->
      <option selected disabled hidden>{$t('reminders.snooze')}</option>
      <!-- options -->
